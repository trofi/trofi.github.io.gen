...
While preparing this release I had a lot of fun fidding with
`autoconf` over past few hours. I hope I did not break it too much to be
generally buildable.
...
and many other conventional tools in the guest `VM`.
...
things to be still broken. It's good enough for proof of concept, but
expect to fix many bugs if you want to run a real application.
...
Previous `Ski` release was in February 2008, 14 years ago. A few things
changed in the world since then. Incomplete list:
...
to get an emulator to support latest kernels to test the patches.
...
I wondered if I could build OS image building based on `nixpkgs` to
...
of packages for `ia64` with one command. `re2c` example:
...
Kernel was slightly harder as it required 2 `CONFIG_` options to be
disabled to get something bootable:
...
magic function which allows doing exactly that.

Just one `rsync` call can be used to build full rootfs! Here is my
...
Just `44MB` uncompressed! If we compress it we'll get `9MB`:
...
That's smaller than uncompressed kernel, which is around `21MB`:
...
It's such a fun toy to fiddle with!
...
real device. And different from `KVM`! A few examples on why `HP_SIM` is
special:

- Separate `bootloader` binary is required to boot `vmlinux`
  (built as part of `linux` kernel)
...
it forever. It would be useful though to get closer to real `ia64` for
interface discovery.
...
`ia64` can still be emulated and be debugged with `Ski`. Both can still
expose generic bugs in various projects.

`Ski` is a great project example of how little one needs to add to
...
it. But I also never had a reason for deeper investigation. It just works!
