...
already explicitly disable parallel builds to avoid known failures:
`groff`, `source-highlight`, `portaudio`, `slang`, `gnu-efi`, `bind`,
`pth`, `libomxil`, `dhcp`, `directfb`, `doxygen`, `gpm`, `judy` and a
few others. That was a good sign.
...
- apply [v4-0001-Add-shuffle-argument-support.patch](https://savannah.gnu.org/bugs/download.php?file_id=52908)
  from <https://savannah.gnu.org/bugs/index.php?62100>
- build `make` as typical `./bootstrap && configure && make`
- use it as `path/to/make --shuffle <your-typical-make-arguments>`
...
It would be nice to get `--shuffle` upstream to ease reporting ordering
problems to other projects without too much detail on how to get
