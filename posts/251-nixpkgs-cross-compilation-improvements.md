...
packages (`mpfr` and the similar) to get past the errro. While it probably
...
Note that `glibc` does not provde `__stack_chk_guard` symbol. In `gcc`'s
case expected to come from `libssp` library we just enabled. `gcc`'s spec
...
Here `configure.ac` just greps `glibc`'s `features.h` header for library
version. It does not do usual linking probing as bootstrap frequently
starts from `gcc` and `glibc` headers alone.

In `nixpkgs`'s case `gcc` build in `cross-compile` case
...
`gcc`'s `./configure` to trick it to use `/${native_system_header_dir}` path.
...
presubmit test told me that I broke `x86_64-darwin` `gcc` build:
...
Normally `gcc`'s `configure.ac` [probes it](https://gcc.gnu.org/git/?p=gcc.git;a=blob;f=gcc/configure.ac;h=446747311a6aec3c810ad6aa4190f7bd383b94f7;hb=HEAD#l6807)
...
bootstrap was broken because `gcc` enables corss-compilation mode
...
in generic include layour scheme `nixpkgs` was using.
...
`gcc`'s `./configure` is surprisingly resilient to all the invalid
