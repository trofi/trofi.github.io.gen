...
No luck. Why did `winedbg` fail? I tried to find something I could
attach `gdb` to:
...
We got a function name! But our backtrace became even worse: only one
...
Somehow `reslen` value in `gdb` backtrace suddenly changed from
